...
def fetch_missing_2023_rounds(rounds):
    print(f"Fetching rounds {rounds} for 2023...")
    
    for round_no in rounds:
        print(f"Fetching Round {round_no}...")
        try:
            url = f"{API_BASE}/2023/{round_no}/results.json"
            ...
            race = Race.query.filter_by(season=2023, round=round_no).first()
            if not race:
                print(f"  Race not found in DB: {race_name}")
                continue
            
            # Clear existing results if any (partial)
...
if __name__ == "__main__":
    with app.app_context():
        # Missing rounds from audit
        rounds = [17, 18, 21, 22] 
        fetch_missing_2023_rounds(rounds)
